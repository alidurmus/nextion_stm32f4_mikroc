_main:
;uartx.c,13 :: 		void main()
;uartx.c,16 :: 		&_GPIO_MODULE_USART2_PA23);
MOVW	R0, #lo_addr(__GPIO_MODULE_USART2_PA23+0)
MOVT	R0, #hi_addr(__GPIO_MODULE_USART2_PA23+0)
PUSH	(R0)
;uartx.c,15 :: 		UART2_Init_Advanced(57600, _UART_8_BIT_DATA, _UART_NOPARITY, _UART_ONE_STOPBIT,
MOVW	R3, #0
MOVW	R2, #0
MOVW	R1, #0
MOVW	R0, #57600
...
ADD	SP, SP, #4
;uartx.c,19 :: 		&_GPIO_MODULE_USART3_PD89);
MOVW	R0, #lo_addr(__GPIO_MODULE_USART3_PD89+0)
MOVT	R0, #hi_addr(__GPIO_MODULE_USART3_PD89+0)
PUSH	(R0)
;uartx.c,18 :: 		UART3_Init_Advanced(57600, _UART_8_BIT_DATA, _UART_NOPARITY, _UART_ONE_STOPBIT,
MOVW	R3, #0
MOVW	R2, #0
MOVW	R1, #0
MOVW	R0, #57600
...
NOP
NOP
;uartx.c,22 :: 		UART2_Write_Text("ali"); //
MOVW	R0, #lo_addr(?lstr1_uartx+0)
MOVT	R0, #hi_addr(?lstr1_uartx+0)
BL	_UART2_Write_Text+0
;uartx.c,24 :: 		EnableInterrupts(); // enable core interrupts
BL	_EnableInterrupts+0
;uartx.c,25 :: 		USART3_CR1bits.RXNEIE = 1; // enable uart rx interrupt
MOVS	R1, #1
SXTB	R1, R1
MOVW	R0, #lo_addr(USART3_CR1bits+0)
MOVT	R0, #hi_addr(USART3_CR1bits+0)
STR	R1, [R0, #0]
;uartx.c,26 :: 		NVIC_IntEnable(IVT_INT_USART3); // enable interrupt vector
MOVW	R0, #55
BL	_NVIC_IntEnable+0
;uartx.c,27 :: 		USART2_CR1bits.RXNEIE = 1; // enable uart rx interrupt
MOVS	R1, #1
SXTB	R1, R1
MOVW	R0, #lo_addr(USART2_CR1bits+0)
MOVT	R0, #hi_addr(USART2_CR1bits+0)
STR	R1, [R0, #0]
;uartx.c,28 :: 		NVIC_IntEnable(IVT_INT_USART2); // enable interrupt vector
MOVW	R0, #54
BL	_NVIC_IntEnable+0
...
; end of _main
_interrupt_usart3:
;uartx.c,83 :: 		void interrupt_usart3() iv IVT_INT_USART3 ics ICS_AUTO {
SUB	SP, SP, #12
STR	LR, [SP, #0]
;uartx.c,87 :: 		if(UART3_Data_Ready()&& i<=6) // If data is received
BL	_UART3_Data_Ready+0
CMP	R0, #0
IT	EQ
BEQ	L__interrupt_usart318
MOVW	R0, #lo_addr(_i+0)
MOVT	R0, #hi_addr(_i+0)
LDRSH	R0, [R0, #0]
CMP	R0, #6
IT	GT
BGT	L__interrupt_usart317
L__interrupt_usart316:
;uartx.c,89 :: 		rx_array[i]=Uart3_Read();
MOVW	R0, #lo_addr(_i+0)
MOVT	R0, #hi_addr(_i+0)
STR	R0, [SP, #8]
LDRSH	R1, [R0, #0]
...
STRH	R0, [R1, #0]
;uartx.c,87 :: 		if(UART3_Data_Ready()&& i<=6) // If data is received
L__interrupt_usart318:
L__interrupt_usart317:
;uartx.c,92 :: 		}
...
_interrupt:
;uartx.c,99 :: 		void interrupt() iv IVT_INT_USART2 ics ICS_AUTO {
SUB	SP, SP, #12
STR	LR, [SP, #0]
;uartx.c,100 :: 		if(UART2_Data_Ready()&& j<=8) // If data is received
...
BGT	L__interrupt20
L__interrupt19:
;uartx.c,102 :: 		rx_array2[j]=Uart2_Read();
MOVW	R0, #lo_addr(_j+0)
MOVT	R0, #hi_addr(_j+0)
STR	R0, [SP, #8]
LDRSH	R1, [R0, #0]
